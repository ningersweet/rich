...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('train_final_6x_fixed.log')
    ]
)
logger = logging.getLogger(__name__)


def fixed_capital_backtest(klines, predictions, initial_balance=1000.0, 
...
def main():
    ...
    logger.info(f"仓位比例: 20% (固定本金)")
    logger.info(f"实际敞口: {cfg.raw['risk']['max_leverage'] * 20}%")
    logger.info(f"策略类型: 固定本金（不复利）")
    ...
    with open(model_dir / 'config.txt', 'w') as f:
        ...
        f.write(f"仓位比例: 20% (固定本金)\n")
        f.write(f"实际敞口: {cfg.raw['risk']['max_leverage'] * 20}%\n")
        f.write(f"策略类型: 固定本金（不复利）\n")
    ...
    results = fixed_capital_backtest(
        klines_backtest, 
        predictions, 
    ...
    logger.info(f"  最大连续亏损: {results['max_consecutive_losses']} 笔")
    logger.info(f"  杠杆倍数: {leverage}x")
    logger.info(f"  实际敞口: {leverage * 20}%")
    
    if results['trades']:
        trades_df = pd.DataFrame(results['trades'])
        output_file = f"backtest/final_6x_fixed_capital_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.csv"
...
if __name__ == '__main__':
    try:
        ...
        print("【最终总结】")
        print("=" * 100)
        print(f"配置: Top-30特征 + 20%固定仓位 + 6倍杠杆")
