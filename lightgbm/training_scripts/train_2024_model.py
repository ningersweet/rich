...
【功能说明】
训练基于2018-2025年历史数据的BTC量化交易模型，用于2026年样本外回测验证。

【训练流程】
1. 加载并过滤2018-2025年K线数据
2. 构建67个技术指标特征
3. 构建风险收益标签（盈亏比、方向、持仓周期）
4. 两阶段训练：
   - 阶段1：使用全部67个特征训练，获取特征重要性排序
   - 阶段2：提取Top30特征重新训练，提高泛化能力
5. 保存模型到 models/final_2025_dynamic/

【数据划分】
- 训练数据：2018-01-01 至 2025-12-31（8年历史数据）
- 回测数据：2026-01-01 至 2026-02-07（样本外验证）
- 严格时间分离，避免数据泄露
